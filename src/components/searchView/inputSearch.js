...
class InputSearch extends Component{
  ...
  fetchData (value) {
    ...
    let filterItems = list.filter((item) => {
      ...
      return (item.city.toLowerCase().search(valueLowercase) >= 0);
    });
    this.setState({
        ...
        {this.state.isOpen ? <div ref={this.toggleContainer} className='ui-input-search-item'>
          <ul className="list-group">
            {this.state.searchedItems.map((item, key) => (
              <li onClick={() => this.selectClickHandler(item.city,item.city)} key={key} className="list-group-item" value={item.city}>
